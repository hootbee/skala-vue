  ...
  <section class="volume-leaders" aria-labelledby="volume-leaders-title">
    ...
    <ol v-else-if="items.length" class="leader-list">
      <li v-for="(item, index) in items" :key="item.symbol">
        <button type="button" :class="{ selected: selectedSymbol === item.symbol }" :aria-pressed="selectedSymbol === item.symbol" @click="emit('select-market', item)">
...
.leader-list button { display: grid; width: 100%; min-height: 104px; grid-template-columns: auto minmax(0, 1fr); align-content: space-between; gap: 9px; padding: 13px; border: 1px solid var(--line); border-radius: 12px; color: var(--ink); background: #fbfdff; text-align: left; cursor: pointer; }
...
.leader-rank { display: grid; width: 24px; height: 24px; place-items: center; border-radius: 7px; color: #fff; background: var(--blue-700); font-size: .7rem; font-weight: 900; }
.leader-name { display: grid; min-width: 0; gap: 3px; }
...
.leader-volume { display: grid; grid-column: 1 / -1; grid-template-columns: auto 1fr; align-items: baseline; gap: 4px; }
...
@media (prefers-reduced-motion: reduce) { .leader-skeletons span { animation: none; } }
</style>
