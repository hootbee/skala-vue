...
const formatValue = (market, value) => `${new Intl.NumberFormat('ko-KR', { maximumFractionDigits: 2 }).format(value)} ${market.currency}`
...
const cardView = computed(() => cards.value.map((card) => ({ ...card, latest: card.series.at(-1)?.value, change: card.series.length > 1 ? card.series.at(-1).value - card.series.at(-2).value : 0, points: chartPoints(card.series) })))

onMounted(async () => {
  await Promise.all(cards.value.map(async (card) => {
    try { const result = await fetchStockSeries(card.market); card.series = result.series; card.source = result.source }
...
<template>
  <main class="stocks-page">
    <header class="stocks-header">
      ...
      <h1>오늘의 주식 시세</h1>
      <p>주요 지수와 관심 종목의 최근 흐름을 간단히 확인해 보세요.</p>
      <small>투자 참고용 정보이며, 실제 매매 전 공식 거래소와 증권사 정보를 확인하세요.</small>
    </header>
    <section class="stock-grid" aria-label="주요 시장 및 종목">
      <article v-for="card in cardView" :key="card.market.id" class="stock-card">
        <div class="stock-card__heading"><div><h2>{{ card.market.name }}</h2><span>{{ card.market.symbol }}</span></div><span v-if="card.source === 'demo'" class="demo-badge">DEMO</span></div>
        ...
        <template v-else>
          <div class="stock-value">{{ formatValue(card.market, card.latest) }}</div>
          <div :class="['change', { down: card.change < 0 }]">{{ card.change >= 0 ? '▲' : '▼' }} {{ Math.abs(card.change).toLocaleString('ko-KR') }} {{ card.market.currency }}</div>
