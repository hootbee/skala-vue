...
export async function handlePostRoutes(request, response, url) {
  ...
  if (request.method === 'GET' && url.pathname === '/api/posts') {
    ...
    const posts = listPosts().filter((post) => !query || post.title.toLowerCase().includes(query) || post.content.toLowerCase().includes(query) || post.author.toLowerCase().includes(query)).toSorted((a, b) => b.id - a.id)
  ...
  if (request.method === 'DELETE' && postMatch) {
    const post = deletePost(Number(postMatch[1]))
